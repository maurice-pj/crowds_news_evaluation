...
# Synthetic graph for the paper
read.csv('synthetic_data.csv') %>%
  mutate(
    true_votes = factor(true_votes, levels = c('Social rating\nrefutes news', 'No social\nrating shown',
                                               'Social rating\nconfirms news')),
    ...
    group_text = factor(group_text, levels = c('No social signal (control)', 'Majority-Democrat group',
                                               'Majority-Republican group')),
    # headline_politics2 = factor(headline_politics, levels = c('Unpolitical social influence',
    #                                                          'Out-group resistance',
    #                                            'Selective attention', 'Signal informativeness')),
    estimate = ifelse(subject_politics == 'Liberal participants', estimate + 0.13, estimate)
  ) %>%
  filter(subject_politics == 'Liberal participants') %>%
ggplot(aes(y=estimate, x=true_votes, color=subject_politics,
             group=subject_politics, shape=group_text)) +
  #geom_e#rrorbar(aes(ymin=lower, ymax=upper), color='black', width=0.1, pos=pos, alpha=0.4) +
  geom_point(size=3, position=pos) +
  geom_line(pos=pos, data = . %>% filter(group %in% c("control" , "maj-dem"))) +
  geom_line(pos=pos, data = . %>% filter(group %in% c("control" , "maj-rep"))) +
  facet_wrap(~headline_politics, nrow=2, ncol=2) +
  scale_color_manual(values=c('#020a7b')) +
  # scale_shape_manual(values = c(15,17,16)) +
  scale_shape_manual(values = c(16,68,82)) +
  scale_y_continuous(limits=c(0.5,0.75), labels = scales::percent_format(accuracy = 5L)) + #limits=c(0.27,.79),
  labs(x='', y='Reported news credibility', color='Participant ideology', shape='Group partisanship') +
